...
namespace MeshLoader
{
	...
	void LoadIndices(ifstream& inFile, Mesh& mesh)	//Lastly, load the indeces from the file which tell how the vertexes are spaced.
	{
		...
		if (mesh.IndexCount > 0)
		{
			...
			{
				inFile >> mesh.Indices[i].vertex;
				mesh.Indices[i].uv = mesh.Indices[i].vertex;
				mesh.Indices[i].normal = mesh.Indices[i].vertex;
				cout << "Vector " << mesh.Indices[i].vertex << ": x: " << mesh.Vertices[i].x << ", y: " << mesh.Vertices[i].y << ", z: " << mesh.Vertices[i].z << endl;
			}
		}
	}
